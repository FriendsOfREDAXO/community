document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateMarkers() {
        ...
        for (var i = 0, max = directory.length; i < max; i++) {
            var grade = directory[i].grade; // Get the grade (e.g., 12, 24, etc.)
            if (grade >= yearRange.min && grade <= yearRange.max) {
                // Generate marker content
                var content = '<div class="user">';
                if (directory[i].image) {
                    content += '<div class="user__image"><img class="user__image-src" src="' + directory[i].image + '" alt=""></div>';
                }
                content += '<div class="user__data">';
                if (directory[i].name) {
                    content += '<h2 class="user__name">' + directory[i].name + '</h2>';
                }
                if (directory[i].bio) {
                    content += '<p class="user__bio">' + directory[i].bio + '</p>';
                }
                if (directory[i].links) {
                    content += '<div class="user__links"><ul class="user__links-list">';
                    for (var j = 0; j < 4; j++) {
                        if (directory[i]['links'][j]) {
                            var link = directory[i]['links'][j];
                            var linkText = link.replace(/(http:\/\/|https:\/\/)/i, '');
                ...
                // Create popup
                var popup = L.popup({ maxWidth: 450 }).setContent(content);
